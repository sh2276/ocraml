...
(** [error_rate lst perceptron] checks every input against the perceptron 
    and calculates what percentage of it is accurately guessed.*)

val train : float -> float -> int -> (Vector.t * 'a) list -> 'a t -> ('a t * float list)
(** [train rate margin num_of_epochs lst perceptron] will train the perceptron either 
    ...
    Whichever is achieved first will return the perceptron. A list of margin of errors 
    is also returned in order of ascending epochs. [rate] is the specified learning rate 
    of the perceptron. *)
